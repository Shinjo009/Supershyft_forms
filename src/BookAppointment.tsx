...
import { getSelectableBookingDates, toIsoDate } from './lib/bookingDates'
import { loadPincodeLookup, lookupPincode } from './lib/pincodeLookup'
import { checkServiceAvailability } from './api/checkServiceAvailability'
import { fetchAvailableSlots, type AvailableSlot } from './api/availableSlots'
...
export default function BookAppointment() {
  ...
  const goNextFromPersonal = async () => {
    ...
    try {
      ...
      if (result.userId != null) {
        ...
        logClientError('User was created but no user id was returned. Please try again.')
        return
      }
      setUiError('')
      setStep(2)
  ...
  const goNextFromSchedule = async () => {
    ...
    if (!form.appointmentTime || !form.appointmentStmId || !form.appointmentSlotTime) {
      logClientError('Please select a time slot.')
      return
    }
    if (!form.gender) {
      logClientError('Gender is required to lock this slot.')
      return
    }
    if (form.userId == null) {
      logClientError('User profile is missing. Please go back to Personal and continue again.')
      return
    }
    ...
    if (!addressLine || !form.city.trim() || !form.pincode.trim()) {
      logClientError('Complete your address before locking a slot.')
      return
    }

    setIsLockingSlot(true)
    try {
      await lockBookingSlot(form.gender, {
        ...
        blood_collection_time_slot_id: form.appointmentStmId,
        blood_collection_time_slot: form.appointmentSlotTime,
      })
      setUiError('')
      setStep(4)
    } catch (error) {
      logClientError(error instanceof Error ? error.message : 'Unable to lock this time slot.')
    } finally {
  ...
  const goNextFromAddress = async () => {
    if (isCheckingServiceability) return
    ...
    if (!form.gender) {
      logClientError('Gender is required to check serviceability.')
      return
    }
    ...
    try {
      const result = await checkServiceAvailability(form.gender, {
        address_line: addressLine,
      ...
      logClientError(
        error instanceof Error ? error.message : 'Unable to check serviceability for this location.',
      )
    } finally {
  ...
  const handleConfirmBooking = async () => {
    ...
    if (!form.appointmentStmId || !form.appointmentSlotTime) {
      logClientError('Please select a time slot.')
      return
    }
    if (!normalizedEmployeeId) {
    ...
    try {
      ...
      // API sometimes returns a success phrase that was previously thrown as an error.
      if (/^booking\s+confirm(ed|ation)?\.?$/i.test(message.trim())) {
        markEmployeeIdAsBooked(normalizedEmployeeId)
        setStep(5)
        return
      }
              ...
              step < 4 ? (
                <div className="mt-auto shrink-0 px-6 pt-4 pb-[30px]">
                  <ContinueButton
                    variant="mobileBar"
                    disabled={
                      (step === 2 && isCheckingServiceability) || (step === 3 && isLockingSlot)
                    }
                    onClick={() => {
                  ...
                  >
                    {step === 2 && isCheckingServiceability
                      ? 'Checking...'
        ...
        {showMobileContinue && (
          <div className="pb-4">
            <ContinueButton
              ...
              onClick={onContinue}
            >
              {isCreatingUser ? 'Saving...' : 'Continue'}
            </ContinueButton>
          </div>
        )}
      </div>
    ...
    return (
      <>
        ...
        <div className="mt-4 flex justify-end">
          <ContinueButton disabled={isCreatingUser} onClick={onContinue}>
            {isCreatingUser ? 'Saving...' : 'Continue'}
          </ContinueButton>
        </div>
      </>
...
function AddressStep({
  ...
  return (
    <div className={`flex flex-col ${isMobile ? 'gap-6' : 'gap-5'}`}>
      ...
      <div className="flex flex-col gap-1">
        {labelRow(Building2, 'Landmark', undefined, isMobile)}
        <input
          className={fieldClass}
...
function ScheduleStep({
  ...
  const availableDates = useMemo(() => getSelectableBookingDates(6), [])
  const [slots, setSlots] = useState<AvailableSlot[]>([])
  const [slotsLoading, setSlotsLoading] = useState(false)
  const [slotsError, setSlotsError] = useState('')

  useEffect(() => {
    const allowed = new Set(availableDates.map((d) => toIsoDate(d)))
    if (form.appointmentDate && !allowed.has(form.appointmentDate)) {
      update('appointmentDate', '')
      update('appointmentTime', '')
      update('appointmentStmId', '')
      update('appointmentSlotTime', '')
    }
  }, [availableDates, form.appointmentDate, update])
  ...
  useEffect(() => {
    ...
    if (!form.gender) {
      setSlots([])
      setSlotsError('Gender is required to load time slots.')
      setSlotsLoading(false)
      return
    }
    ...
    if (!addressLine || !form.city.trim() || !form.pincode.trim()) {
      setSlots([])
      setSlotsError('Complete your address to load time slots.')
      setSlotsLoading(false)
      return
    }
    ...
    void (async () => {
      try {
        const nextSlots = await fetchAvailableSlots(form.gender, {
          address_line: addressLine,
          landmark: form.landmark.trim(),
          city: form.city.trim(),
          pincode: form.pincode.trim(),
        ...
        setSlots([])
        setSlotsError(
          error instanceof Error ? error.message : 'Unable to load time slots for this date.',
        )
        update('appointmentTime', '')
        update('appointmentStmId', '')
        update('appointmentSlotTime', '')
  ...
  const selectedDateClass =
    'bg-[radial-gradient(50.74%_50.76%_at_50%_50%,_#11795F_0%,_#1C493D_100%)] border-transparent text-white'
  const idleDateClass = 'border-white/[0.08] bg-white/5 text-[#9a9a9a]'
  ...
  const selectDate = (iso: string) => {
    if (form.appointmentDate === iso) return
  ...
  const selectSlot = (slot: AvailableSlot) => {
    update('appointmentTime', slot.label)
    update('appointmentStmId', slot.stmId)
    update('appointmentSlotTime', slot.slotTime)
  }
  ...
  return (
    <div className={`flex flex-col items-start self-stretch ${isMobile ? 'gap-6' : 'gap-9'}`}>
      <section className={`flex flex-col items-start self-stretch ${isMobile ? 'gap-3' : 'gap-6'}`}>
        ...
        <div className="-mx-1 w-[calc(100%+8px)] overflow-x-auto px-1 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            ...
            {availableDates.map((date) => {
              ...
              return (
                <button
                  ...
                  className={[
                    'flex h-[72px] w-[72px] shrink-0 flex-col items-center justify-center gap-1 rounded-[12px] border transition',
                    selected ? selectedDateClass : idleDateClass,
                    showMissingRequired && !form.appointmentDate ? 'border-[#ff6b6b]/40' : '',
                  ].join(' ')}
                >
                  <span className="text-[12px] font-medium leading-none">
                    {DAY_LABELS[date.getDay()]}
                  </span>
                  <span className="text-[20px] font-semibold leading-none">{date.getDate()}</span>
                </button>
              )
            })}
          </div>
      ...
      <section className={`flex flex-col items-start self-stretch ${isMobile ? 'gap-3' : 'gap-6'}`}>
        ...
        {!form.appointmentDate ? (
          <p className="text-[12px] font-light text-[#999]">Select a preferred date to see available time slots.</p>
        ) : slotsLoading ? (
          <p className="text-[12px] font-light text-[#999]">Loading available time slots…</p>
        ) : slotsError ? (
          <p className="text-[12px] font-light text-[#ff6b6b]">{slotsError}</p>
        ) : slots.length === 0 ? (
          <p className="text-[12px] font-light text-[#999]">No time slots available for this date.</p>
        ) : (
          <div className={isMobile ? 'grid w-full grid-cols-2 gap-2' : 'grid w-full grid-cols-2 gap-4 lg:grid-cols-3'}>
            {slots.map((slot) => {
              const selected = form.appointmentTime === slot.label
              return (
                <button
                  key={slot.stmId || `${slot.slotTime}-${slot.endTime}`}
                  type="button"
                ...
                >
                  <span className={selected ? 'text-white' : 'text-[#9a9a9a]/80'}>{slot.label}</span>
                </button>
              )
            })}
          </div>
